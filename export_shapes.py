...
lista_horas = ['%s:00:00' % h for h in xrange(16,23)]
...
for k, fecha in enumerate(lista_fechas):
    for i,h in enumerate(lista_horas):
        if i < len(lista_horas) -1:
            sql = "select c.id, c.uname, c.text, to_char(c.hora,'HH24:MI:SS') as hora, c.geom from tweets_rectificado c where c.fecha = '" + fecha +\
                    "' and (c.hora >= '" + h + "' and c.hora <= '" + \
                    lista_horas[i+1]+ "')"
            nombre_shape = "/home/plablo/twitter_afternoons/d_"+str(k)+ "_t_"  + str(i) +".shp"
